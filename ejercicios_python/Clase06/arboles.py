...
archivo_arboles = os.path.join('ejercicios_python','Data','arbolado-en-espacios-verdes.csv')
...
AD = [(float(arbol['altura_tot']), float(arbol['diametro'])) for arbol in arboleda if arbol['nombre_com'] == 'Jacarandá']
# pprint(AD)
...
def medidas_de_especies(lista_especies, lista_arboleda):
    return {especie: [(float(arbol['altura_tot']), float(arbol['diametro'])) for arbol in lista_arboleda if arbol['nombre_com'] == especie] for especie in lista_especies}
...
archivo_arboles = os.path.join('ejercicios_python','Data','arbolado-en-espacios-verdes.csv')
arboleda = leer_arboles(archivo_arboles)
# altos = [comprensión de listas]       Ej 5.16
def histograma_jacarandas(lista):
    plt.hist(lista, bins=12, color='lightgreen', edgecolor='black')
    plt.show()
# histograma_jacarandas(H)
...
def scatter_hd_varios(dicc_medidas):
    plt.figure(figsize=(12, 8))
    
    for especie, lista_de_pares in dicc_medidas.items():
        array = np.array(lista_de_pares)
        h = array[:, 0]
        d = array[:, 1]
        
        plt.scatter(d, h, alpha=0.5, label=especie)
    
    plt.xlabel("diametro (cm)")
    plt.ylabel("alto (m)")
    plt.title("Relación diámetro-alto para varias especies")
    plt.legend()
